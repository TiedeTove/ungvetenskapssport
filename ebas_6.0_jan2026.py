...
##################################################################################################
##################################################################################################
filepath = os.path.join(r"C:\Users\Tove Tiedemann\Downloads", filename) #filename = ebas-månad.csv
##################################################################################################
##################################################################################################
...
plt.title(f'{months[month-1].capitalize()} 2026', fontsize='xx-large') ### ändra årtal här ###
plt.tight_layout()
plt.show()
...
##################################################################################################
##################################################################################################
referens = pd.read_csv(r"C:\Users\Tove Tiedemann\Documents\Bachelor - UU\UVS\data-koordinater.csv", delimiter=";", encoding="ISO-8859-1", on_bad_lines="skip", encoding_errors="replace")
##################################################################################################
##################################################################################################
...
##################################################################################################
##################################################################################################
world = gpd.read_file(r"C:\Users\Tove Tiedemann\Documents\Bachelor - UU\UVS\ne_10m_admin_0_countries.shp")
##################################################################################################
##################################################################################################
...
ncols = 4
nrows = 2
fig_all_static, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(20, 4 * nrows))
...
for idx, target_forening in enumerate(forening_list):
    ...
    ax_static.set_title(forening_titles.get(target_forening, target_forening), fontsize=14, loc='center')
    ...
    # Format new cities with line breaks every 2 cities, or force breaks before/after long names (>10 chars) or Hallstahammar
    city_lines = []
    current_line = []
    for city in truly_new_cities:
        is_long = len(city) > 10 or city == "Hallstahammar"
        # If current line has items and we hit a long name, finish current line first
        if current_line and is_long:
            city_lines.append(", ".join(current_line))
            current_line = []
        # Add city to current line
        current_line.append(city)
        # If this is a long name or we have 2 cities, finish the line
        if is_long or len(current_line) == 2:
            city_lines.append(", ".join(current_line))
            current_line = []
    if current_line:
        city_lines.append(", ".join(current_line))
...
# Adjust specific subplot positions
# Index mapping: Astro=2, Ingenjör=6, Biologi=3, Matte=7
adjust_positions = {
    3: 0.03, # Biologer: move left
    7: 0.03  # Matematiker: move left
}
for idx, x_offset in adjust_positions.items():
    pos = axes[idx].get_position()
    new_pos = [pos.x0 + x_offset, pos.y0, pos.width, pos.height]
    axes[idx].set_position(new_pos)

plt.tight_layout()
plt.subplots_adjust(left=-0.06)  # Move all subplots and content to the left
##################################################################################################
##################################################################################################
plt.savefig(r"C:\Users\Tove Tiedemann\Desktop\medlemmar_alla_static_test.png", bbox_inches='tight') #var du vill spara din fil
##################################################################################################
##################################################################################################
plt.show()
...
for target_forening in forening_list:
    # Use cumulative counts from the bar chart data
    forening_idx = target_föreningar.index(target_forening)
    ...
    fig, (ax, ax_graph) = plt.subplots(1, 2, figsize=(16, 8))
    ...
    # Setup cumulative graph on the right
    ax_graph.set_xlabel('Månad')
    ax_graph.set_ylabel('Medlemmar')
    month_labels = [' '] + [m[:3].capitalize() for m in months[:month]]
    ax_graph.set_xticks(range(len(month_labels)))
    ax_graph.set_xticklabels(month_labels, rotation=45)
    ax_graph.grid(True, alpha=0.3)
    line, = ax_graph.plot([], [], color=custom_color, linewidth=2, marker='o')
    ...
    def get_sizes(month):
        ...
        for stad in stad_names:
            ...
            counts.append(count)
        sizes = 30 * np.log10(np.array(counts) + 1) + 10
        return sizes
    ...
    def update(frame):
        ...
        # Use cumulative count from bar chart data
        total_members = int(data[forening_idx][frame])
        fig.suptitle(f'Medlemmar i {pretty_name}: {months[frame]} (Totalt: {total_members})', fontsize=14)
        
        # Update cumulative graph - start with point at 0
        x_data = list(range(frame+2))
        y_data = [0] + [int(data[forening_idx][i]) for i in range(frame+1)]
        line.set_data(x_data, y_data)
        ax_graph.set_ylim(0, max(y_data) * 1.1 if y_data else 1)
        ax_graph.set_xlim(-0.5, len(month_labels)-0.5)
...
# Setup cumulative graph on the right for all organisations
ax_graph.set_xlabel('Månad')
ax_graph.set_ylabel('Medlemmar')
month_labels_all = [' '] + [m[:3].capitalize() for m in months[:month]]
ax_graph.set_xticks(range(len(month_labels_all)))
ax_graph.set_xticklabels(month_labels_all, rotation=45)
ax_graph.grid(True, alpha=0.3)
line, = ax_graph.plot([], [], color='blue', linewidth=2, marker='o')
...
def update_all(frame):
    ...
    scat.set_sizes(sizes)
    # Calculate total from all organisations
    total_members = int(sum(data[:, frame]))
    fig.suptitle(f"Medlemmar i Sverige: {months[frame]} (Totalt: {total_members})", fontsize=14)
    
    # Update cumulative graph - start with point at 0
    x_data = list(range(frame+2))
    y_data = [0] + [int(sum(data[:, i])) for i in range(frame+1)]
    line.set_data(x_data, y_data)
    ax_graph.set_ylim(0, max(y_data) * 1.1 if y_data else 1)
    ax_graph.set_xlim(-0.5, len(month_labels_all)-0.5)
...
##################################################################################################
##################################################################################################
ani_all.save(r"C:\Users\Tove Tiedemann\Desktop\medlemmar_sverige_alla.gif", writer="pillow") #var du vill spara din gif
##################################################################################################
##################################################################################################
plt.close(fig)
